...
static WifiInfo *vtx_nic_get_wpa_info(const gchar *interface)
{
  ...
  JsonObject *status = status_output ? vtx_nic_parse_wpa_cli_command(status_output) : NULL;
  g_free(status_output);

  gchar *signal_output = vtx_nic_run_wpa_cli_command(interface, "signal_poll");
  JsonObject *signal = signal_output ? vtx_nic_parse_wpa_cli_command(signal_output) : NULL;
