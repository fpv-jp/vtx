...
// --- vtx_add_rtp_header_extensions_with_list ----------------------------------
static void
vtx_add_rtp_header_extensions_with_list (GstElement * payloader, const gchar ** uris, guint count)
{
  guint id = 1;
  for (guint i = 0; i < count; i++) {
    ...
    if (!ext) {
      gst_printerr ("Failed to create RTP header extension for URI: %s\n", uri);
      continue;
    }
...
void
vtx_add_video_rtp_header_extensions (GstElement * videopay)
{
  vtx_add_rtp_header_extensions_with_list (videopay, rtp_video_header_extensions, G_N_ELEMENTS (rtp_video_header_extensions));
  g_clear_object (&videopay);
}

void
vtx_add_audio_rtp_header_extensions (GstElement * audiopay)
{
  vtx_add_rtp_header_extensions_with_list (audiopay, rtp_audio_header_extensions, G_N_ELEMENTS (rtp_audio_header_extensions));
  g_clear_object (&audiopay);
}

// --- vtx_priority_from_string ----------------------------------
static GstWebRTCPriorityType
vtx_priority_from_string (const gchar * s)
{
...
void
vtx_set_transceiver_priority (GArray * transceivers, const gchar * video_priority, const gchar * audio_priority)
{
  ...
  if (video_priority) {
    GstWebRTCPriorityType priority = vtx_priority_from_string (video_priority);
    if (priority) {
  ...
  if (audio_priority) {
    GstWebRTCPriorityType priority = vtx_priority_from_string (audio_priority);
    if (priority) {
      GstWebRTCRTPSender *sender = NULL;
      g_object_get (trans, "sender", &sender, NULL);
      gst_webrtc_rtp_sender_set_priority (sender, priority);
