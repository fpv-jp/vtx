...
UnityPrintFloat (const UNITY_DOUBLE input_number)
{
  ...
  if (number == 0.0f) {
    ...
    decimals = ((exponent <= 0)
        && (exponent >= -(sig_digits + 3))) ? (-exponent) : (sig_digits - 1);
    exponent += decimals;
...
UnityAssertBits (const UNITY_INT mask,
    ...
    const UNITY_INT actual, const char *msg, const UNITY_LINE_TYPE lineNumber)
{
...
UnityAssertEqualString (const char *expected,
    const char *actual, const char *msg, const UNITY_LINE_TYPE lineNumber)
{
  UNITY_UINT32 i;
